...
# ── Main flow (WazeProtocolSource.queryArea) ──────────────────────────────────

def _bootstrap(lat: float, lon: float, debug: bool, alert_cache: dict) -> "WazeSession":
    """Full register → login → prepare_for_area. Retries up to 3 times on 504."""
    for full_attempt in range(3):
        sess = WazeSession(lat, lon, debug=debug)
        try:
            sess.register(lat, lon)
        except Exception as exc:
            raise RuntimeError(f"REGISTER: {exc}")

        try:
            _, login_batch = sess.login(lat, lon)
            for a in _parse_alerts(login_batch):
                key = a.get("uuid") or str(a.get("id")) or None
                if key:
                    alert_cache[key] = {k: v for k, v in a.items()}
        except Exception as exc:
            raise RuntimeError(f"LOGIN: {exc}")

        # Handshake — retry the full bootstrap on persistent 504
        try:
            handshake_batch = sess.prepare_for_area(lat, lon)
            for a in _parse_alerts(handshake_batch):
                key = a.get("uuid") or str(a.get("id")) or None
                if key:
                    alert_cache[key] = {k: v for k, v in a.items()}
            # Check if handshake still erroring
            still_error = any(el.HasField("error") for el in handshake_batch.element)
            if still_error and full_attempt < 2:
                if debug:
                    print(f"  handshake still erroring after retries — "
                          f"full re-bootstrap attempt {full_attempt+2}", file=sys.stderr)
                time.sleep(5)
                continue
            return sess
        except Exception as exc:
            if full_attempt < 2:
                if debug:
                    print(f"  handshake exception {exc} — re-bootstrapping", file=sys.stderr)
                time.sleep(5)
                continue
            raise RuntimeError(f"HANDSHAKE: {exc}")

    return sess  # last attempt


def run(lat: float, lon: float, radius_km: float, debug: bool = False) -> list:
    """Query Waze RT alerts for a circle. Raises on failure (never calls sys.exit)."""
    alert_cache: dict = {}
    sess = _bootstrap(lat, lon, debug, alert_cache)  # raises on failure
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Waze RT client — method 2 (mobile binary protocol)"
    )
    ...
    try:
        alerts = run(args.lat, args.lon, args.radius_km, debug=args.debug)
        print("REGISTER: OK\nLOGIN: OK\nQUERY: OK")
    except Exception as exc:
